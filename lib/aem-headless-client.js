...
/**
   ...
   * @returns a GraphQL response of all adventures.
   */
 async getRockStarHomePage() {
  const randomNumberForCacheIssue = Math.floor(100000 + Math.random() * 900000);
  const rockstarHomePage = process.env.NEXT_PUBLIC_AEM_GRAPHQL_ENDPOINT_ROCKSTAR + '/rockstar-home-page?' + randomNumberForCacheIssue;
   
  console.log(rockstarHomePage);
  try {
   return await this.aemHeadlessClient.runPersistedQuery(rockstarHomePage);
